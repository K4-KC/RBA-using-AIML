...
for file in range(1):
    ...
    for index, row in sql_data.iterrows():
        ...
        add.append((row['User ID'], 
                    ...
                    np.array(get_one_hot_encoding(country, row['Country']), dtype=np.int8).tobytes(),
                    np.array(get_one_hot_encoding(region, row['Region']), dtype=np.int8).tobytes(),
                    np.array(get_one_hot_encoding(city, row['City']), dtype=np.int8).tobytes(),
                    np.array(get_one_hot_encoding(Asn, row['ASN']), dtype=np.int8).tobytes(),
                    np.array(get_one_hot_encoding(Device_type, row['Device Type']), dtype=np.int8).tobytes(),
...
print("Device Type", np.frombuffer(first_entry[7], dtype=np.int8))
print("Login Successful", first_entry[8])
print("Is Attack IP", first_entry[9])
print("Is Account Takeover", first_entry[10])
