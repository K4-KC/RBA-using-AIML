...
- Data alignment and consolidation from multiple CSV sources into a unified dataset.
- Exploration and one-hot encoding of categorical features for downstream modeling. 
- Extraction of attacked-user records to build focused attack datasets.
- Splitting of large datasets into multiple subsets for scalable processing and experimentation.
- Training of machine learning models for real-time or near–real-time account takeover detection.
...
The project is organized around several standalone scripts, which can be run step by step or individually depending on your workflow.
...
etc.
text
3. Inspect console output and any generated files (such as CSVs or model artifacts) to verify that each step behaves as expected. 
